...
bool ImGui::Tab(bool selectable, const char* label, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, selectable ? c::text::text_active : hovered ? c::text::text_hov : c::text::text, g.IO.DeltaTime * c::tabs::animation_speed);

    it_anim->second.text_offset = ImClamp(it_anim->second.text_offset + (6.f * g.IO.DeltaTime * (selectable ? 10.f : -10.f)), 0.f, 24.f);

    it_anim->second.arrow_opticaly = ImLerp(it_anim->second.arrow_opticaly, selectable ? 1.f : 0.f, g.IO.DeltaTime * c::tabs::animation_speed);

    GetWindowDrawList()->AddCircleFilled(ImVec2(bb.Min.x + 8.f, bb.Min.y + 10.f), 3.f, GetColorU32(c::accent_color, it_anim->second.arrow_opticaly), 100.f);

    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + it_anim->second.text_offset, bb.Max.y - CalcTextSize(label).y - (size.y - CalcTextSize(label).y) / 2), GetColorU32(it_anim->second.text), label);
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? c::accent_color : c::button::background, g.IO.DeltaTime * 6.f);

    ImGui::GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.background), c::button::rounding + 3);

    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(c::button::outline), c::button::rounding, NULL, 1.f);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    window->DrawList->AddRectFilled(bb_frame.Min, bb_frame.Max, GetColorU32(c::button::background), 30.f, ImDrawCornerFlags_BotRight);
    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
...
{
    float opticaly = 0.f, size_accent;
};

void ImGui::ProgressBar(ImGuiID id, float fraction, const ImVec2& size_arg, const char* overlay)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(c::progress::background, 0.5f), c::progress::rounding);
    GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(c::progress::outline), c::progress::rounding);
    ...
    static std::map<ImGuiID, progress_state> anim;
    auto it_anim = anim.find(id);
   
    if (it_anim == anim.end())
    {
        anim.insert({ id, progress_state() });
        it_anim = anim.find(id);
    }
   
    it_anim->second.size_accent = ImLerp(it_anim->second.size_accent, fraction > 1.00f ? (size.x - 6) : 0.f, g.IO.DeltaTime * 6.f);

    if (fraction > 0.01f)
    GetWindowDrawList()->AddRectFilled(bb.Min + ImVec2(6, 6), bb.Min + ImVec2(fraction * size.x - it_anim->second.size_accent - 6, size.y - 6), GetColorU32(c::accent_color), c::progress::rounding);
    ...
    RenderTextClipped(ImVec2(ImClamp(fill_br.x, bb.Min.x, bb.Max.x - overlay_size.x), bb.Min.y), bb.Max, overlay, NULL, &overlay_size, ImVec2(0.5f, 0.5f), &bb);
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    it_anim->second.line_offset = ImLerp(it_anim->second.line_offset, IsItemActive() ? size_arg.y : 0.f, g.IO.DeltaTime * 6.f);
    it_anim->second.stoke_color = ImLerp(it_anim->second.stoke_color, IsItemActive() ? c::accent_color : c::input::outline, g.IO.DeltaTime * 6.f);
    ...
    if (!is_multiline)
    {
        ...
        ImGui::PushClipRect(frame_bb.Min + ImVec2(0, it_anim->second.line_offset), frame_bb.Max, true);
        GetWindowDrawList()->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(it_anim->second.stoke_color), c::input::rounding, NULL, IsItemActive() ? 2.f : 1.f);
        ImGui::PopClipRect();

    }

    const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + inner_size.x, frame_bb.Min.y + inner_size.y); // Not using frame_bb.Max because we have adjusted size
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + ImVec2((size_arg.y - CalcTextSize(hint).y) / 2, (size_arg.y - CalcTextSize(hint).y) / 2);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor)
        {
            ...
            it_anim->second.slow_input = ImLerp(it_anim->second.slow_input, cursor_screen_rect.Min.x - total_bb.Min.x, g.IO.DeltaTime * 20.f);

            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(ImVec2(total_bb.Min.x + it_anim->second.slow_input, cursor_screen_rect.Min.y), ImVec2(total_bb.Min.x + it_anim->second.slow_input, cursor_screen_rect.GetBL().y), GetColorU32(c::text::text_active));
